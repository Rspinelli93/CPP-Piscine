...
/// @brief Bureaucrat member function. It will try to sign a AForm
/// @param f AForm to sign
/// @return void
/// @throws AForm::GradeTooLowException if grade is too low to sign
void Bureaucrat::signAForm( AForm &f )
{
	try
	{
		f.beSigned(*this);
		std::cout << this->getName() << " signed: " << f.getName() << " AForm\n" << std::endl;
	}
...
void Bureaucrat::executeForm(AForm const &form) const
{
	...
	{
		form.execute(*this);
		std::cout << "Bureaucrat: " << this->getName() << " executed " << form.getName() << " form" << std::endl;
	}
	catch(const std::exception& e)
	{
		std::cerr << "Bureaucrat: " << this->getName() << " can't execute " << form.getName() << " form: " << e.what() << std::endl;
	}
}